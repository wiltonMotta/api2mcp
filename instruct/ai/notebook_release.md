...
- **URL 路由说明**：释放等状态变更操作直连集群 `{aiUrls}` 以获得更低延迟（与 create/start 使用 AC 平台 URL 不同）
...
注：MCP 参数 `notebook_id` 映射到 OpenAPI 的 `id`（**注意：上游 API release/rename/startService 使用 `id`，而 stop 使用 `notebookId`，这是上游 API 自身的差异，不是文档错误**）。
...
3. **构造请求**：
   - URL: `POST {aiUrl}/ai/openapi/v2/notebook/actions/release`
...
- **API 返回错误**：返回 `{"error": true, "message": "释放 Notebook 失败 [{code}]: {msg}"}`
- **网络异常**：返回 `{"error": true, "message": "释放 Notebook 请求异常: {详情}"}`
...
- 将工具描述文档写入 `APIs` 表，name 为 `notebook_release`
- document JSON 包含 url、method、description、parameters、returns

## 代码位置

`main.py` 中新增 `@mcp.tool()` 装饰的 `notebook_release` 函数。
