...
- **URL 路由说明**：关机等状态变更操作直连集群 `{aiUrls}` 以获得更低延迟（与 create/start 使用 AC 平台 URL 不同）
...
|----------|------|------|--------|-------------|---------|------|
| `notebook_id` | string | 是 | — | `notebookId` | body | notebook 实例 ID。可从 `notebook_list` 返回的 `id` 字段获取 |
| `save_env` | boolean | 否 | `False` | `saveEnv` | body | 是否保存当前运行环境 |
| `cluster_id` | integer | 否 | `None` | — | — | 集群 ID。为空时使用默认集群 |

注：MCP 参数 `notebook_id` 映射到 OpenAPI 的 `notebookId`（**注意：上游 API stop 使用 `notebookId`，而 release/rename 使用 `id`，这是上游 API 的差异**）；`save_env` 映射到 `saveEnv`。
...
3. **构造请求**：
   - URL: `POST {aiUrl}/ai/openapi/v2/notebook/actions/stop`
   ...
   - Body: `{"notebookId": "...", "saveEnv": true/false}`
...
- **API 返回错误**：返回 `{"error": true, "message": "关机 Notebook 失败 [{code}]: {msg}"}`
- **网络异常**：返回 `{"error": true, "message": "关机 Notebook 请求异常: {详情}"}`
...
- 将工具描述文档写入 `APIs` 表，name 为 `notebook_stop`
- document JSON 包含 url、method、description、parameters、returns

## 代码位置

`main.py` 中新增 `@mcp.tool()` 装饰的 `notebook_stop` 函数。
