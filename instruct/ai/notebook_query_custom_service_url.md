...
实现一个 MCP tool `notebook_query_custom_service_url`，查询指定 Notebook 实例中用户自定义服务的访问地址（非 JupyterLab 服务，如用户自行启动的 WebUI 等）。

## 前置条件

- 用户需先完成 AK/SK 认证（`/auth/{username}`），确保 `user_cluster` 表中存在有效的集群 token
- 需要先调用 `notebook_start_custom_service` 启动自定义服务
...
3. **构造请求**：
   - URL: `GET {aiUrl}/ai/openapi/v2/notebook/customize-service/url`
...
- **前置操作**：
  - 需先调用 `notebook_start_custom_service` 启动自定义服务
  - `notebook_start_custom_service` → `notebook_query_custom_service_url`（启动后查询访问地址）
- **同类工具**：`notebook_query_jupyter_url` — 查询 JupyterLab 服务地址

## 异常处理

- **未认证**：返回 `{"error": true, "message": "未找到认证信息...", "auth_url": "..."}`
- **缺少 notebook_id**：返回 `{"error": true, "message": "notebook_id 为必填参数"}`
- **实例不可访问**：`status` 为 `inactive` 时，建议提示用户先调用 `notebook_start_custom_service`
- **API 返回错误**：返回 `{"error": true, "message": "查询自定义服务地址失败 [{code}]: {msg}"}`
- **网络异常**：返回 `{"error": true, "message": "查询自定义服务地址请求异常: {详情}"}`
...
```json
{
    ...
    "data": {
        "status": "active",
        "url": "https://c-1896476639463936002.ksai.scnet.cn:58043"
    }
}
```
...
- 将工具描述文档写入 `APIs` 表，name 为 `notebook_query_custom_service_url`
- document JSON 包含 url、method、description、parameters、returns

## 代码位置

`main.py` 中新增 `@mcp.tool()` 装饰的 `notebook_query_custom_service_url` 函数。
