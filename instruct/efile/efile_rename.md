...
实现一个 MCP tool `efile_rename`，重命名 HPC 集群文件系统上的文件。
...
|----------|------|------|--------|-------------|---------|------|
| `path` | string | 是 | - | `fileAbsolutePath` | body (form-urlencoded) | 源文件绝对路径 |
| `new_name` | string | 是 | - | `newName` | body (form-urlencoded) | 文件修改后的新名称（仅文件名，不含路径） |
...
3. **构造请求**：
   - URL: `POST {efileUrl}/efile/openapi/v2/file/rename`
   - Header: `token: {clusterToken}`
   - Content-Type: `application/x-www-form-urlencoded`
   - Body: `fileAbsolutePath={path}&newName={new_name}`
...
- Body (form-urlencoded):
  - `fileAbsolutePath` — 源文件绝对路径（来自 MCP 参数 `path`）
  - `newName` — 新文件名（来自 MCP 参数 `new_name`）
- 超时: 15s
...
- **网络异常**：返回 `{"error": true, "message": "重命名文件请求异常: {详情}"}`
