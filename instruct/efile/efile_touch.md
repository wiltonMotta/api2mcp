...
实现一个 MCP tool `efile_touch`，在 HPC 集群文件系统上创建空文件。
...
|----------|------|------|--------|-------------|---------|------|
| `path` | string | 是 | - | `fileAbsolutePath` | body (form-urlencoded) | 要创建的文件的绝对路径（含文件名） |
...
3. **构造请求**：
   - URL: `POST {efileUrl}/efile/openapi/v2/file/touch`
   - Header: `token: {clusterToken}`
   - Content-Type: `application/x-www-form-urlencoded`
   - Body: `fileAbsolutePath={path}`
...
- Body (form-urlencoded):
  - `fileAbsolutePath` — 要创建的文件的绝对路径（来自 MCP 参数 `path`）
- 超时: 15s
...
- **网络异常**：返回 `{"error": true, "message": "创建文件请求异常: {详情}"}`
