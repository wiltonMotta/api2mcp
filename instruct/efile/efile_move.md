...
实现一个 MCP tool `efile_move`，在 HPC 集群文件系统上移动文件。支持批量移动（多个源文件逗号分隔）。
...
- **网络异常**：返回 `{"error": true, "message": "移动文件请求异常: {详情}"}`
